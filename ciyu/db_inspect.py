...
def print_samples(conn=None, limit_main=10, limit_rel=20):
    ...
    try:
        ...
        cur.execute('SELECT id, word, pinyin, synonyms, antonyms FROM hanyuguoxue_ciyu ORDER BY id DESC LIMIT %s', (limit_main,))
        rows = cur.fetchall()
        for r in rows:
            print(r)

        _print_title('词语关系表（样例）')
        cur.execute('SELECT id, min_id, max_id, relation_type FROM ciyu_relation ORDER BY id DESC LIMIT %s', (limit_rel,))
